...
def tela_cliente():
    ...
    while True:
        ...
        for codigo, info in cardapio.items():
            print(f"[{codigo}] {info['item']} - R$ {info['preco']:.2f} ")
        print("[0] Cancelar Compra e Sair do Menu ")

        try:
            opcao_produto = int(input("\nDigite o número do item (ou 0 para cancelar/voltar): "))
        except ValueError:
        ...
        print("\n" + "-" * 30)
        proximo_passo = input("Deseja [A]dicionar mais um produto ou [F]inalizar e pagar? (A/F): ").upper()
        if proximo_passo == "F":
            break

    if len(carrinho) > 0:
        print("\n" + "-" * 45)
        print("          RESUMO DO SEU CARRINHO         ")
        print("-" * 45)
        for produto in carrinho:
        ...
        if confirmar == "S":
            ...
            dados_todas_lojas["Unidade Matriz (Loja 01)"]["venda_atual"] += total_pedido
            ...
            tipo_envio = "Retirada"
            if opcao_meio == 1:
                print("\n COMO DESEJA RECEBER SEU PEDIDO DO APP? ")
                ...
                if opcao_envio == 2:
                    tipo_envio = "Delivery"
                    endereco = input("Digite seu endereço e número para entrega: ")
                    print("\n[Dados protegido conforme Lei Geral da Proteção de Dados (LGPD)]")
            ...
            time.sleep(1)
            print("[STATUS]: Seu Pedido está Pronto!")

            if tipo_envio == "Delivery":
                print("\n [STATUS FINAL]: Seu pedido saiu para entrega!")
            else:
                print("\n [STATUS FINAL]: Retirar seu pedido no balcão!")
        else:
            for produto in carrinho:
                estoque[produto['item']] += 1
            print("\nPedido Cancelado.")
...
def tela_funcionario(nome_usuario, loja_vinculada):
    ...
    while True:
        ...
        print("1. Ver Estoque Local desta Unidade")
        print("2. Ver Metas e Indicadores desta Unidade")
        print("3. Registrar Cancelamento")
        print("0. Sair e Retornar para Tela Inicial ")
...
def tela_chefe():
    ...
    while True:
        ...
        if opcao == "0":
            ...
            print("\n QUADRO DE FUNCIONÁRIOS DA REDE ")
            for nome_loja, dados in dados_todas_lojas.items():
